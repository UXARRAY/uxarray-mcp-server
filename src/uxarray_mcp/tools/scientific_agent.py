...
def _decide_venue(file_path: str, n_face: int) -> str:
    ...
    """
    if any(file_path.startswith(prefix) for prefix in _HPC_PATH_PREFIXES):
        return "hpc"
...
def run_scientific_agent(
    ...
    # ── STAGE 1: ANALYZE ──────────────────────────────────────────────────────
    reasoning_trace.append({"stage": "analyze", "action": f"inspect_mesh({file_path!r})"})
    ...
    reasoning_trace.append(
        {
            ...
            "reason": (
                ...
                if any(file_path.startswith(p) for p in _HPC_PATH_PREFIXES)
    ...
    if total_area is not None:
        # Global meshes should be ~5.1e14 m²; allow wide range for subsets
        if total_area > 0:
            reasoning_trace.append(
    ...
    if zonal_mean_results:
        ...
        finite_values = [v for v in values if v is not None and v == v]  # filter NaN
        if len(finite_values) < len(values):
            n_nan = len(values) - len(finite_values)
            warnings.append(f"zonal mean contains {n_nan} NaN/null values")
            reasoning_trace.append(
                {"stage": "verify", "check": "zonal_mean finite", "result": f"{n_nan} NaNs"}
            )
        else:
            reasoning_trace.append(

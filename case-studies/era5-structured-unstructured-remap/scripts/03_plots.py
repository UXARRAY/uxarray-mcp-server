...
pcm = ax.pcolormesh(
    lon, lat, field, cmap="Blues", shading="auto", vmin=0, vmax=field.max(),
...
plt.close(fig)
print("wrote plot_era5_original.png")
...
fig, axes = plt.subplots(
    1, len(methods), figsize=(5.2 * len(methods), 5),
...
for ax, method in zip(axes, methods):
    ...
    ax.set_extent([lon.min(), lon.max(), lat.min(), lat.max()], crs=ccrs.PlateCarree())
    pcm = ax.pcolormesh(
        lon, lat, diff, cmap="RdBu_r", shading="auto", vmin=-vmax, vmax=vmax,
        transform=ccrs.PlateCarree(),
    )
    ax.coastlines(resolution="50m")
    ax.add_feature(cfeature.BORDERS.with_scale("50m"), linestyle=":")
...
plt.close(fig)
print("wrote plot_roundtrip_bias.png with", len(methods), "methods:", methods)
